...
### Lead Machine profiles

`POST /api/v1/scrape` accepts three MarketPulse Lead Machine profiles in addition to the GeoSerra profiles:

- `website-analysis`: returns company identity, contact hints, social links, B2B/product/private-label/China signals, and visible text for AI analysis.
- `directory-listing`: returns normalized `companies[]` from directory-like listing pages such as Europages or TOBB pages.
- `fair-exhibitor`: returns normalized `exhibitors[]` with booth/stand hints when present.
