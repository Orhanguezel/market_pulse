...
async function saveRiskScore(tenantKey: string, jobId: string, report: AmazonRiskReport) {
  const id = randomUUID();
  await pool.execute(
    `INSERT INTO amazon_risk_scores (
      id, tenant_key, job_id, keyword,
    ...
    [
      id, tenantKey, jobId, report.keyword,
...
async function saveAmazonProducts(tenantKey: string, jobId: string, products: AmazonProduct[]) {
  for (const product of products) {
    await pool.execute(
      `INSERT INTO amazon_products (
        id, tenant_key, job_id, title, price, rating, review_count, seller_name, seller_url, product_url, asin
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        randomUUID(),
        tenantKey,
...
async function saveAmazonScanJob(tenantKey: string, jobId: string, keyword: string, marketplace: string) {
  await pool.execute(
    `INSERT IGNORE INTO amazon_scan_jobs (id, tenant_key, keyword, marketplace, status) VALUES (?, ?, ?, ?, 'running')`,
    [jobId, tenantKey, keyword, marketplace],
  );
}

async function updateAmazonScanJob(tenantKey: string, jobId: string, patch: { status: string; dataPoints?: number; errorMsg?: string | null }) {
  await pool.execute(
    `UPDATE amazon_scan_jobs SET status = ?, data_points = COALESCE(?, data_points), error_msg = ?, finished_at = CURRENT_TIMESTAMP WHERE tenant_key = ? AND id = ?`,
    [patch.status, patch.dataPoints ?? null, patch.errorMsg ?? null, tenantKey, jobId],
  );
}

async function logAmazonJobError(tenantKey: string, jobId: string, errorMessage: string): Promise<number> {
  const [rows] = await pool.execute(
    `SELECT COUNT(*) AS count FROM amazon_job_error_logs WHERE tenant_key = ? AND job_id = ?`,
    [tenantKey, jobId],
  );
  ...
  await pool.execute(
    `INSERT INTO amazon_job_error_logs (id, tenant_key, job_id, error_type, error_msg, retry_count) VALUES (?, ?, ?, ?, ?, ?)`,
    [randomUUID(), tenantKey, jobId, errorType, errorMessage, retryCount],
  );
  return retryCount;
}
...
export async function runAmazonJob(jobId: string) {
  ...
  await saveAmazonScanJob(tenantKey, jobId, keyword, marketplace);
  ...
  try {
    ...
    await saveAmazonProducts(tenantKey, jobId, eligible);
    ...
    await saveRiskScore(tenantKey, jobId, report);
    ...
    await updateAmazonScanJob(tenantKey, jobId, { status: 'done', dataPoints: report.data_points });
    await updateSearchJob(jobId, { status: 'done', resultCount: 1, finished: true });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'UNKNOWN_ERROR';
    try {
      await logAmazonJobError(tenantKey, jobId, msg);
    } catch {
      // log tablosu hatası, ana job hata yönetimini engellememeli
    }
    await updateAmazonScanJob(tenantKey, jobId, { status: 'failed', errorMsg: msg });
